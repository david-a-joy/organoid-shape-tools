...
def add_violinplot(ax: Axes, data, x, y, hue=None, significance=None, orient='v',
                   order=None, hue_order=None, palette=COLOR_PALETTE,
                   sig_barcolor=None, sig_linewidth=5, bottom=0.0,
                   sig_fontsize=24, sig_markers=None, capsize=0.2, errcolor=None,
                   savefile=None, plot_individual_samples=False,
                   plot_sample_numbers=True,
                   **kwargs):
    """ Add a seaborn-style violinplot with extra decorations

    :param Axes ax:
        The matplotlib axis to add the barplot for
    :param DataFrame data:
        The data to add a violinplot for
    :param str x:
        The column to use for the categorical values
    :param str y:
        The column to use for the real values
...
def add_lineplot(ax: Axes,
    ...
    :param str palette:
        The palette to use
    :param str color:
        If not None, override palette and always plot lines with this color
    :param Path savefile:
    ...
    for cat in order:
        for hue_cat in hue_order:
            ...
            if n_samples >= 3:
                ...
                ymean = np.mean(ydata)
                ylow, yhigh = bootstrap_ci(ydata, n_boot=n_boot)
            elif n_samples >= 1:
                catdata = data[mask]
                ydata = catdata[y].values
    ...
    for i, hue_cat in enumerate(hue_order):
        ...
        if err_style in ('band', 'bands'):
            ax.fill_between(xcoords, ylow, yhigh, facecolor=hue_color, alpha=0.5)
            ax.plot(xcoords, ymean, '-', color=hue_color, label=hue_label)
        elif err_style in ('bar', 'bars'):
            ax.errorbar(xcoords, ymean, np.stack([ymean-ylow, yhigh-ymean], axis=0),
                        capsize=15, linewidth=3, color=hue_color, label=hue_label)
        else:
