...
class CompartmentPlot(object):
        ...
        with set_plot_style(self.plot_style) as style:
            ...
            if mode == 'split':
                for i in range(self.n_compartments):
                    ...
                    ax.fill_between(self._xdata, value_st*yscale, value_ed*yscale,
                                    facecolor=self.colors[i], alpha=0.5)
                    ax.plot(self._xdata, value_mean*yscale, '-', color=self.colors[i], linewidth=2)
            elif mode == 'all':
                ...
                ax.plot(self._xdata, value_mean*yscale, '-', color='b', linewidth=2)
            else:
    ...
    def load_plotdata(self, infile):
        """ Reload the plotdata from disk """

        print(f'Loading {infile}')
        if infile.suffix in ('.csv', ):
            plotdata = pd.read_csv(infile)
        elif infile.suffix in ('.xlsx', ):
            plotdata = pd.read_excel(infile)
        else:
            raise KeyError('Unknown plot data input file type: {}'.format(infile))

        columns = plotdata.columns
        xcolumn = str(columns[0])

        ycolumn = str(columns[1])
        assert ycolumn.startswith('mean')

        ycolumn = ycolumn[len('mean'):]

        if ycolumn.endswith('all'):
            ycolumn = ycolumn[:-len('all')]
        elif ycolumn.endswith('bin1'):
            ycolumn = ycolumn[:-len('bin1')]
        else:
            raise ValueError(f'Invalid ycolumn name "{ycolumn}"')

        ycolumn = ycolumn.strip()
        print(f'{ycolumn}')
        self._xcolumn = xcolumn
        self._ycolumn = ycolumn
        self._xdata = plotdata[xcolumn].values

        total_count = np.zeros_like(self._xdata)
        for i in range(self.n_compartments):
            label = 'count {} bin{}'.format(self._ycolumn, i+1)
            total_count += plotdata[label].values
