...
* :py:func:`nearest_vertex_of_polyhedron`: Given a collection of test points, find the vectors to the vertex points
...
class GridValueExtractor(object):
    ...
    def segment_peaks(self,
        ...
        """
        min_distance = max([1, int(np.round(min_distance))])
...
def nearest_vertex_of_polyhedron(volume_points: np.ndarray, test_points: np.ndarray) -> np.ndarray:
    """ Compute the vector to the nearest vertex of a polyhedron

    https://stackoverflow.com/questions/29880880/shortest-distance-from-a-point-to-a-face-of-a-polyhedron

    :param ndarray volume_points:
        The array of m x 3 points comprising the entire volume
    :param ndarray test_points:
        The array of k x 3 points to find the vector to the surface for
        (must be INSIDE the hull of volume_points)
    :returns:
        A k x 3 array of **unnormalized** vectors from the test points to the nearest surface point
    """
    assert volume_points.ndim == 2
    assert volume_points.shape[0] > 3
    assert volume_points.shape[1] == 3

    assert test_points.ndim == 2
    assert test_points.shape[1] == 3

    # Calculate the exterior vertex points
    hull = ConvexHull(volume_points)
    vertex_points = volume_points[hull.vertices, :]

    # Use a ball tree to find the nearest vertex point for each test point
    tree = BallTree(vertex_points)
    inds = tree.query(test_points, k=1, return_distance=False)

    # Return the vector from the test point to the vertex point
    return vertex_points[inds[:, 0], :] - test_points
