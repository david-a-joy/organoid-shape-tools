...
def to_json_types(data):
    ...
    if isinstance(data, str):
        ...
        return str(data)
    elif isinstance(data, (int, bool, np.bool_, np.int64, np.int32, np.uint8)):
        return int(data)
...
def load_image(imgfile: pathlib.Path, ctype: str = 'gray') -> np.ndarray:
    ...
    """
    ctype = ctype.lower()
    img = np.asarray(Image.open(imgfile)).astype(np.float64)
...
def save_image(imgfile: pathlib.Path,
    ...
    if 'quality' not in save_kwargs:
        if imgfile.suffix in ('.jpg', '.jpeg'):
            save_kwargs['quality'] = 100
    # if 'compression' not in save_kwargs:
    #     if imgfile.suffix in ('.tif', '.tiff'):
    #         save_kwargs['compression'] = 'zstd'
    img.save(str(imgfile), **save_kwargs)
