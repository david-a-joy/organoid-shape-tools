...
# Our own imports
from organoid_shape_tools.utils import movie_utils
from .. import helpers
...
class TestWriteMovie(helpers.FileSystemTestCase):
    ...
    def test_write_movie_controls_video_codec(self):
        ...
        with movie_utils.av.open(str(outfile), mode='r') as container:
            video_stream = container.streams.video[0]
            self.assertEqual(video_stream.codec_context.name, 'h264')
            self.assertEqual(video_stream.codec_context.type, 'video')

        outfile.unlink()

        outfile = self.tempdir / 'movie.mp4'
        movie_utils.write_movie([o1, o1, o1, o1, o1], outfile, video_codec='mpeg4')
        self.assertTrue(outfile.is_file(), f'{outfile} not found')

        with movie_utils.av.open(str(outfile), mode='r') as container:
            video_stream = container.streams.video[0]
            self.assertEqual(video_stream.codec_context.name, 'mpeg4')
            self.assertEqual(video_stream.codec_context.type, 'video')
